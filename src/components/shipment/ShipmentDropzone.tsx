...
import { FileSpreadsheet, UploadCloud, X, CheckCircle2, AlertCircle, Loader2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";

export type ShipmentFileStatus = "queued" | "processing" | "done" | "error";

export interface QueuedShipmentFile {
  id: string;
  file: File;
...
export function ShipmentDropzone({
  files,
  onAdd,
  onRemove,
  onProcess,
  ...
  const handleFiles = useCallback(
    (list: FileList | null) => {
      if (!list) return;
      const validFiles = Array.from(list).filter(
        (f) =>
      ...
      if (validFiles.length) onAdd(validFiles);
    },
    [onAdd],
  );
    ...
    <div className="mx-auto w-full max-w-4xl px-4 sm:px-6">
      ...
      >
        ...
        <p className="mt-5 font-display text-2xl">
          {dragging
            ? "Drop your shipment spreadsheets here"
            : "Drag & drop your shipment Excels or CSVs"}
        </p>
        <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
          Upload shipping manifests, carrier dispatch lists or courier exports (.xlsx, .csv) — we
          auto-detect headers, extract orders, and merge them into your master fleet.
        </p>
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="truncate text-xs text-muted-foreground">
                        ...
                        {f.status === "processing" && "Extracting rows…"}
                        {f.status === "done" && `${f.records ?? 0} shipments extracted`}
            ...
            <div className="mt-5 flex justify-center">
              ...
              >
                {busy
                  ...
                  : `Process Shipments (${files.filter((f) => f.status === "queued").length})`}
              </Button>
            </div>
          </motion.div>
        )}
