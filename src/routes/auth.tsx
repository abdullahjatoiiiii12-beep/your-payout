...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      { title: "Sign In & Sign Up — YourPayouts" },
      {
        name: "description",
        content:
          "Sign in or register to securely sync and store your payout PDFs and shipment order records.",
      },
      { property: "og:title", content: "Sign In & Sign Up — YourPayouts" },
    ],
  }),
  component: AuthPage,
});
...
function AuthPage() {
  return (
    <div className="relative min-h-screen bg-background text-foreground selection:bg-primary selection:text-primary-foreground flex flex-col justify-between overflow-x-hidden">
      {/* Background Decorators matching theme */}
      ...
      <main className="relative z-10 mx-auto w-full max-w-5xl px-4 pt-8 pb-32 sm:px-6 sm:pt-14 sm:pb-28 flex-1 flex flex-col items-center justify-center">
        ...
        {/* Auth Form Box with entry animation */}
        <motion.div
        ...
        >
          <AuthForm defaultTab="signin" redirectTo="/dashboard" />
        </motion.div>
